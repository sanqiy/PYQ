...
(function(window, document) {
	...
	Cupload.prototype = {
		...
		initDom: function() {
			...
			this.createOverlay()
			if (Array.isArray(this.opt.data) && this.opt.data.length > 0) {
		...
		createUploadBox: function() {
			...
			this.uploadBox.style = 'margin-left:15px'
			...
			this.uploadBox.style.backgroundColor = 'var(--backbg)'
			//this.uploadBox.style.border = '1px dashed #c0ccda'
			this.uploadBox.style.borderRadius = '2px'
			...
			this.uploadBox.onmouseover = function() {
				_this.uploadBox.style.borderColor = '#07c160'
			}
		...
		createImageBox: function(src, name, width, height, state = true) {
			...
			this.imageBox[this.i].draggable = 'true'
			this.imageBox[this.i].style.position = 'relative'
			this.imageBox[this.i].style.display = 'inline-block'
			/*this.imageBox[this.i].setAttribute('onmousedown', 'down()')
			this.imageBox[this.i].setAttribute('onmouseup', 'up()')*/
		...
		createImagePreview: function(src, width, height) {
			...
			this.imagePreview[this.i].setAttribute('data-src', src)
			this.imagePreview[this.i].onerror = function() {
				this.onerror = null
				this.removeAttribute('data-src')
				this.src = '/assets/img/thumbnail.svg'
			}
			//this.setImageAttribute(width, height)
			this.imagePreview[this.i].style= 'width:100%;height:100%;object-fit: cover;pointer-events:none;'
		...
		createImageInput: function(src) {
			...
			this.imageInput[this.i].style.pointerEvents = 'none'
		...
		createImageDelete: function(name) {
			...
			/*this.imageDelete[this.i].style.display = 'none'*/
			...
			this.imageDelete[this.i].style.pointerEvents = 'none'
		...
		createDeleteBtn: function() {
			...
			/*this.deleteBtn[this.i].style.display = 'none'*/
			this.deleteBtn[this.i].style.position = 'absolute'
			this.deleteBtn[this.i].style.pointerEvents = 'all'
			this.deleteBtn[this.i].style.top = '5px'
			this.deleteBtn[this.i].style.right = '5px'
		...
		createSortBtn: function() {
			...
			this.sortLeftBtn[this.i].style.pointerEvents = 'all'
			//this.sortLeftBtn[this.i].style.display = 'none'
			this.sortLeftBtn[this.i].style.bottom = '5px'
			this.sortLeftBtn[this.i].style.left = '5px'
			...
			this.sortRightBtn[this.i].style.pointerEvents = 'all'
			//this.sortRightBtn[this.i].style.display = 'none'
			this.sortRightBtn[this.i].style.bottom = '5px'
			this.sortRightBtn[this.i].style.right = '5px'
...
function yidong(){
    //电脑-----------------------------------------------------
    if (document.querySelector('.cupload-image-list')) {
        // 获取列表dom
        let list = document.querySelector('.cupload-image-list');
        ...
        list.ondragenter = e => {
            ...
            if (tiemListIndex < targetIndex) {
                ...
                //console.log('往上移动');
                if (e.target.parentNode === list) {//如果参考元素不是当前元素的子元素，就会抛出DOMException错误 所以这里需要确保target元素是list的子元素
...
list.addEventListener('touchmove', e => {
  ...
  // 实时更新被拖动元素的位置
  if (currentItem.className != "cupload-image-box") {//判断拖动按下的一瞬间的元素是不是li不是则不执行
      return;
  }
  currentItem.style.position="absolute";
  currentItem.style.left = touch.clientX-50 + 'px';
  currentItem.style.top = touch.clientY-50 + 'px';
...
list.addEventListener('touchend', e => {
  ...
  if (target === currentItem || target === list) {
    return;
  }

  let itemList = Array.from(list.children);
  let currentIndex = itemList.indexOf(currentItem);
  let targetIndex = itemList.indexOf(target);

  if (currentIndex < targetIndex) {
    //console.log('往下移动');
    list.insertBefore(currentItem, target.nextElementSibling);
  } else {
    //console.log('往上移动');
    if (currentItem && target && target.parentNode === list) {//如果参考元素不是当前元素的子元素，就会抛出DOMException错误 所以这里需要确保target元素是list的子元素
        list.insertBefore(currentItem, target);
    }
  }
